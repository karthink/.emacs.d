...
(defcustom gptel-inline-prompt-position t
  "Where in the chat buffer to insert gptel-inline's prompt.

Three options are available:
t or end: insert at end of the chat buffer (default)
last: insert after the last completed response
nil: insert at point in the chat buffer
...
(defvar-local gptel-inline--prompt-position nil
  "Marker in the gptel buffer where the prompt is inserted.")
...
(defun gptel-inline--prompt-position (&optional buffer)
  "Return position at which to insert prompt in BUFFER."
  (with-current-buffer (or buffer (current-buffer))
    (if (markerp gptel-inline--prompt-position)
        gptel-inline--prompt-position
      (setq gptel-inline--prompt-position (make-marker))
      (set-marker-insertion-type gptel-inline--prompt-position t)
      (move-marker
       gptel-inline--prompt-position
       (pcase gptel-inline-prompt-position
         ('last
          (if-let* (((gptel-fsm-p gptel--fsm-last))
                    (info (gptel-fsm-info gptel--fsm-last))
                    (pos (plist-get info :tracking-marker)))
              (save-excursion
                (save-restriction
                  (goto-char pos)
                  (when (looking-at
                         (regexp-quote (concat gptel-response-separator
    ...
    (when (and (overlayp tco) (overlay-buffer tco))
      ...
      (gptel-inline--update-response-overlay
       (with-current-buffer (overlay-buffer tco)
         ...
         (concat "\n" (buffer-substring (1- (overlay-start tco))
                                        (1- (overlay-end tco)))))
       response-ov)
      ;; Update keybindings
...
CONTEXT-PLIST is the context plist for this inline session.  Returns the
...
show the next response."
  ...
  (let* ((origin (point-marker))
    ...
    (with-current-buffer prompt-buf
      (goto-char (gptel-inline--prompt-position gptel-buf))
...
be called from the `gptel-inline' prompt buffer."
  (interactive
   (let ((cb (current-buffer)))         ;the inline prompt buffer
     (list (read-buffer                 ;all gptel buffers except this one
            "Response in session: "
            gptel-inline-default-chat-buffer nil
            (lambda (b) (let ((buf (get-buffer (or (car-safe b) b))))
                     (and (buffer-local-value 'gptel-mode buf)
                          (not (eq buf cb)))))))))
...
(defun gptel-inline-append (&optional no-quit)
  "Append the inline prompt and any reference text to the chat buffer.
...
(defun gptel-inline-send ()
  "Send the gptel inline prompt to the LLM.
  ...
  (let* ((prompt-buf    (current-buffer))
    ...
    (with-current-buffer gptel-buf
      (if delimited                     ;Send only prompt (no buffer context)
        ...
        (goto-char (gptel-inline--prompt-position)))
      (setq gptel-inline--prompt-position nil)
